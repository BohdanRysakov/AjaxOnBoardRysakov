package rys.ajaxpetproject.message.infractructure.grpc
...
import rys.ajaxpetproject.message.domain.entity.toModel
...
class MessageGrpcService(
    private val messageCreateUseCase: IMessageCreateInPort,
    private val chatAddMessageUseCase: ChatAddMessageAdapter,
    private val messageEventService: IMessageAddedEventPublishInPort
    ...
    override fun create(request: MessageCreateRequest): Mono<MessageCreateResponse> {
        return request.message.toModel().let {
            messageCreateUseCase.create(it)
        }.flatMap {
            chatAddMessageUseCase.addMessage(it.id!!, request.chatId)
        }
