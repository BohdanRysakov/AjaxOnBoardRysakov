...
@WebMvcTest(controllers = arrayOf(UserController::class))
//@SpringBootTest
...
class UserControllerTest {
    ...
    fun getUserSuccessfullyTest() {
        ...
        mockMvc.perform(get("/users/${testUser.id}")
    ...
    fun getUserFailDueToBadRequestTest() {
        ...
        mockMvc.perform(get("/users/SomeString")
            .contentType(MediaType.APPLICATION_JSON))
            .andExpect(status().isBadRequest)
    }
    @Test
    fun createUserFailDueToValidationTest() {
        val testUser = User(UUID.randomUUID(), "12",
            "www")
        ...
        mockMvc.perform(post("/users/")
            ...
            .andExpectAll(jsonPath("message",
                anyOf(
                    `is`("Password must be between 8 and 100 characters, " +
                    "Username must be between 3 and 20 characters"),
                    `is`("Username must be between 3 and 20 characters," +
